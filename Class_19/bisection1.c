...
int main ()
{
float a=0.01, b=1.4*M_PI, x, delta_x=1.e-3/*0.001*/,funkca, funkcb, funkcx;
int k = 0;

funkca=sin(a); funkcb=sin(b);
if(funkca*funkcb>0)
{
printf("Intervalaa [%.2f:%.2f] sin(x) funkcijai ",a,b);
...
k++;//k=k+1;//k+=1;
x=(a+b)/2.;
if(funkca*sin(x)>0) //pie a=0 -> funkca=0 -> reizinajums ir precizi 0
			// visu laiku -> visu laiku strada b=x
